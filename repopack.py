...
from textual.widgets._tree import TreeNode, Tree # For Tree.NodeSelected type hint
...
class CheckableDirectoryTree(DirectoryTree):
    BINDINGS = [
        ...
        Binding("s", "toggle_selection_recursive_cursor", "Select Item Recursively (S)", show=False, priority=True), # CHANGED HERE
    ]

    class SelectionChanged(Message): # Keep as is
    ...
    def __init__(self, path: str, id: Optional[str] = None, ignored_patterns: Optional[List[str]] = None): # Keep as is
    ...
    def filter_paths(self, paths: Iterable[Path]) -> Iterable[Path]: # Keep as is
        for path_obj in paths:
            if not self._is_path_ignored(path_obj): yield path_obj

    def _is_path_ignored(self, path_obj: Path) -> bool: # Keep as is
    ...
    def _is_node_effectively_selected_file(self, file_path: Path) -> bool: # Keep as is
    ...
    def render_label(self, node: TreeNode[DirEntry], base_style: Style, style: Style) -> Text: # Keep as is
    ...
    def _toggle_single_node_selection(self, node_fs_path: Path): # Keep as is
    ...
    def _toggle_node_and_children_selection(self, node_fs_path: Path): # Keep as is
    ...
    def _apply_selection_recursive(self, node_fs_path: Path, select_state: bool): # Keep as is
        if select_state:
    ...
    def action_toggle_expand_or_select(self) -> None: # Bound to Enter (Keep as is)
    ...
    def action_space_pressed_on_item(self) -> None: # Bound to Space (Keep as is)
        if self.cursor_node and self.cursor_node.data:
            node_fs_path = self.cursor_node.data.path
    ...
    def on_tree_node_selected(self, event: Tree.NodeSelected[DirEntry]) -> None: # For Clicks (Keep as is)
        event.stop() 
    ...
    async def on_key(self, event: events.Key) -> None: # Keep as is
        ...
        # 's' will be handled by its binding, so we don't need to intercept it here.
        if key_handled_by_us: event.prevent_default()

    def get_selected_final_files(self) -> List[Path]: # Keep as is
...
# --- PathInputScreen class (Keep as is) ---
...
# --- RepoPackerApp class (Keep as is) ---
class RepoPackerApp(App[None]):
    ...
    BINDINGS = [
        ...
        Binding("ctrl+a", "select_in_focused_folder", "Sel Content (Dir)", show=True), # This is different from 's' in tree
        Binding("ctrl+d", "deselect_in_focused_folder", "Desel Content (Dir)", show=True), # This is different from 's' in tree
    ...
    def action_select_all_tree(self) -> None: # App-level select all
        try:
            tree = self.query_one(CheckableDirectoryTree)
    ...
    def action_deselect_all_tree(self) -> None: # App-level deselect all
        try:
            tree = self.query_one(CheckableDirectoryTree)
    ...
    def _operate_on_focused_folder_contents(self, select_state: bool): # For Ctrl+A, Ctrl+D in app
        try:
            tree = self.query_one(CheckableDirectoryTree)
            if tree.cursor_node and tree.cursor_node.data:
                node_path = tree.cursor_node.data.path
                if node_path.is_dir(): 
                    # This specifically selects/deselects CHILDREN of the focused folder,
                    # not the folder itself. If you want to include the folder, modify _apply_selection_recursive
                    # or call _toggle_node_and_children_selection on node_path.
                    # For now, let's assume it means children only for Ctrl+A/Ctrl+D.
    ...
    def action_select_in_focused_folder(self) -> None: # App-level Ctrl+A
        self._operate_on_focused_folder_contents(select_state=True)
    def action_deselect_in_focused_folder(self) -> None: # App-level Ctrl+D
